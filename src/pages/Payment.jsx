...
// Complete Working Payment Page with Firebase Auto ID

import { useEffect, useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { formatPrice, TRANSPORT_TYPES } from "../data/transports";
import { addBooking } from "../firebase/bookingsService";
import {
  FaCreditCard,
  FaWallet,
  FaMobile,
...
import { MdConfirmationNumber, MdDashboard, MdPayment } from "react-icons/md";
...
const METHODS = [
  { id: "bkash", name: "bKash", icon: "🩷", color: "border-pink-500", bg: "bg-pink-50", text: "text-pink-600", darkBg: "dark:bg-pink-900/20", darkText: "dark:text-pink-400", type: "wallet" },
  { id: "nagad", name: "Nagad", icon: "🧡", color: "border-orange-500", bg: "bg-orange-50", text: "text-orange-600", darkBg: "dark:bg-orange-900/20", darkText: "dark:text-orange-400", type: "wallet" },
  { id: "mobile", name: "Mobile Banking", icon: "📱", color: "border-purple-500", bg: "bg-purple-50", text: "text-purple-600", darkBg: "dark:bg-purple-900/20", darkText: "dark:text-purple-400", type: "wallet" },
  { id: "card", name: "Card", icon: "💳", color: "border-blue-500", bg: "bg-blue-50", text: "text-blue-600", darkBg: "dark:bg-blue-900/20", darkText: "dark:text-blue-400", type: "card" },
];
...
export default function Payment() {
  ...
  useEffect(() => {
    ...
    if (pending) {
      setBooking(pending);
      console.log("📋 Pending booking loaded:", pending.id);
    } else {
  ...
  // ---------- 🔥 FIXED: Payment Confirm with Firebase Auto ID ----------
  const confirmPayment = async () => {
    ...
    try {
      ...
      // 🔥 STEP 1: Save to Firebase FIRST (get Auto ID)
      const result = await addBooking(paidBooking);
      ...
      // 🔥 STEP 2: Get Firebase Auto ID
      const firebaseId = result.id;
      console.log("✅ Firebase Auto ID:", firebaseId);

      // 🔥 STEP 3: Save to localStorage with Firebase Auto ID
      const finalBooking = { ...paidBooking, id: firebaseId };
      
      let existingBookings = [];
      try {
        existingBookings = JSON.parse(localStorage.getItem("stb-bookings") || "[]");
      } catch (e) {
        existingBookings = [];
      }
      
      existingBookings.push(finalBooking);
      localStorage.setItem("stb-bookings", JSON.stringify(existingBookings));
      localStorage.removeItem("stb-pending-booking");
      
      console.log("✅ Booking saved to localStorage with ID:", firebaseId);
      console.log("📦 Total bookings:", existingBookings.length);

      // 🔥 STEP 4: Navigate with Firebase Auto ID
      navigate(`/ticket/${firebaseId}`);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="mt-8 flex items-center justify-center gap-2">
          <div className={`flex items-center gap-2 ${step >= 1 ? 'text-indigo-600' : 'text-gray-400'}`}>
            <span className={`flex h-8 w-8 items-center justify-center rounded-full font-bold ${step >= 1 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-500 dark:bg-gray-700'}`}>1</span>
            <span className="text-sm font-medium">Details</span>
          </div>
          <div className={`h-0.5 w-12 ${step >= 2 ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'}`} />
          <div className={`flex items-center gap-2 ${step >= 2 ? 'text-indigo-600' : 'text-gray-400'}`}>
            <span className={`flex h-8 w-8 items-center justify-center rounded-full font-bold ${step >= 2 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-500 dark:bg-gray-700'}`}>2</span>
            <span className="text-sm font-medium">Confirm</span>
          </div>
          <div className={`h-0.5 w-12 ${step >= 3 ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'}`} />
          <div className={`flex items-center gap-2 ${step >= 3 ? 'text-indigo-600' : 'text-gray-400'}`}>
            <span className={`flex h-8 w-8 items-center justify-center rounded-full font-bold ${step >= 3 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-500 dark:bg-gray-700'}`}>3</span>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2">
            <div className="rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800 animate-fade-in-up">
              ...
              <div className="mt-4 space-y-3 text-sm">
                ...
                <div className="flex items-start gap-3">
                  <FaMapMarkerAlt className="mt-0.5 text-indigo-500" />
                  <div>
                ...
                <div className="flex items-start justify-end gap-3 text-right">
                  ...
                  <FaLocationArrow className="mt-0.5 text-purple-500" />
                </div>
            ...
            {/* Passenger Info */}
            <div className="mt-4 rounded-2xl bg-white p-4 shadow-lg dark:bg-gray-800 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {METHODS.map((m) => (
                  ...
                  >
                    <p className={`text-3xl group-hover:scale-110 transition-transform ${method === m.id ? 'scale-110' : ''}`}>
                      {m.icon}
                    </p>
                ...
                <div className="mt-6 animate-fade-in-up">
                  <div className={`rounded-xl ${selected.bg} p-4 ${selected.darkBg}`}>
                    <p className={`flex items-center gap-2 text-sm font-bold ${selected.text} ${selected.darkText}`}>
                      <FaInfoCircle />
                      {selected.name} Payment Instructions:
                    </p>
              ...
              {/* Error */}
              {error && (
                <div className="mt-5 flex items-start gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-400 animate-fade-in-up">
              ...
              <button
                ...
                disabled={processing}
                className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-3.5 text-sm font-semibold text-white transition hover:shadow-lg hover:shadow-indigo-500/25 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {processing ? (
                  <>
                  ...
                  <>
                    <FaLock />
                    Confirm Payment — {formatPrice(booking.total)}
                    <FaArrowRight className="text-xs" />
                  </>
                )}
              </button>

              <p className="mt-4 flex items-center justify-center gap-1.5 text-center text-xs text-gray-500 dark:text-gray-400">
